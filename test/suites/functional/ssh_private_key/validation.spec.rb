  ...
  describe ':: Resource' do
    describe ':: ssh_private_key' do
      describe ':: Validation' do
        ...
        it 'does not run validation by default' do |test_case|
          properties = {
            type: 'ssh-rsa',
            user: 'jodie',
            content: 'aabbccdd'
        ...
        it 'raises exception in case of missing ssh-keygen' do |test_case|
          doubler = double(run_command: nil, error?: true, stdout: nil, stderr: nil)
          matcher = receive(:new).with('which', 'ssh-keygen').and_return(doubler)
          allow(Mixlib::ShellOut).to matcher
        ...
        it 'successfully runs validation against single private key' do |test_case|
          type = 'ssh-rsa'
          properties = {
            user: 'jodie',
            content: 'aabbccdd',
        ...
        it 'successfully runs validation against key pair' do |test_case|
          type = 'ssh-rsa'
          public_key = 'aabbccdd'
          properties = {
            user: 'jodie',
        ...
        it 'raises exception on type mismatch' do |test_case|
          type = 'ssh-rsa'
          public_key = 'aabbccdd'
          properties = {
            user: 'jodie',
        ...
        it 'raises exception on public key mismatch' do |test_case|
          type = 'ssh-rsa'
          public_key = 'aabbccdd'
          properties = {
            user: 'jodie',
