  ...
  describe ':: Resource' do
    describe ':: ssh_private_key' do
      describe ':: Behavior' do
        ...
        it 'installs only private key by default' do |test_case|
        ...
        it 'does not create public key for installation if one is already provided' do |test_case|
        ...
        it 'creates public key for installation if none is provided' do |test_case|
          properties = resource.clone
          properties[:install_public_key] = true
          stub_data_bag_item('ssh-private-keys', 'default').and_return(properties)
        ...
        it 'uses additional attributes if specified' do |test_case|
          properties = {
            user: 'jodie',
            content: private_key,
            type: type,
        ...
        it 'uses additional public key attributes if specified' do |test_case|
          properties = {
            user: 'jodie',
            content: private_key,
            type: type,
        ...
        it 'does not invoke ssh-keygen if type is not supplied' do |test_case|
          expect(Mixlib::ShellOut).not_to(
            receive(:new).with('/usr/bin/ssh-keygen', any_args)
          )
        ...
        it 'invokes ssh-keygen if type is not supplied and public key installation requested' do |test_case|
          properties = resource.clone
          properties[:install_public_key] = true
          stub_data_bag_item('ssh-private-keys', 'default').and_return(properties)
